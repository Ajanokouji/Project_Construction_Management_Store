...
namespace _3.GUI.View.FromSanPham
{
	public partial class FrmSanPhamChiTiet : Form
	{
		...
		public void loadDuLieu()
		{
			...
			{
				dtgv_sanPham.Rows.Add(item.SanPhamChiTiets.IDSanPhamChiTiet, item.SanPhamChiTiets.maSP, item.SanPhamChiTiets.TenSP, item.SanPhamChiTiets.Gianhap, item.SanPhamChiTiets.Giaban, item.SanPhamChiTiets.Soluong, item.hangSXs.tenHangSX, item.sanPhams.tenLoaiHang,
					item.SanPhamChiTiets.Trangthai == true ? "Còn hàng" : "Hết hàng");
			}
			dtgv_sanPham.Columns[0].Visible = false;
		}
		...
		private bool checkValidate()
		{
			if (tbt_ten.Text.Trim() == "" || tbt_giaNhap.Text.Trim() == ""
				|| tbt_giaBan.Text.Trim() == "" || tbt_soLuong.Text.Trim() == "" || cb_NSX.Text.Trim() == "" || cbb_loaiHang.Text.Trim() == "") return false;
			return true;
		}
		...
		{
			int row = e.RowIndex;
			if (row == _qLsanPhamCTServices.getViewSanPhamCT().Count) return;
			tbt_maSP.Text = dtgv_sanPham.Rows[row].Cells[1].Value.ToString();
			tbt_ten.Text = dtgv_sanPham.Rows[row].Cells[2].Value.ToString();
			tbt_giaNhap.Text = dtgv_sanPham.Rows[row].Cells[3].Value.ToString();
			tbt_giaBan.Text = dtgv_sanPham.Rows[row].Cells[4].Value.ToString();
			tbt_soLuong.Text = dtgv_sanPham.Rows[row].Cells[5].Value.ToString();
			//cb_Mau.Text = row.Cells[6].Value.ToString();
			//cb_size.Text = row.Cells[7].Value.ToString();
			cb_NSX.Text = dtgv_sanPham.Rows[row].Cells[6].Value.ToString();
			cbb_loaiHang.Text = dtgv_sanPham.Rows[row].Cells[7].Value.ToString();
			rb_con.Checked = dtgv_sanPham.Rows[row].Cells[8].Value.ToString() == "Còn hàng" ? true : false;
			rb_het.Checked = dtgv_sanPham.Rows[row].Cells[8].Value.ToString() == "Hết hàng" ? true : false;
		...
		//	|| x.hangSXs.tenHangSX.ToLower().Contains(tbt_timKiem.Text)
		//	 || x.sanPhams.tenLoaiHang.ToLower().Contains(tbt_timKiem.Text)
